document.addEventListener('DOMContentLoaded', () => {
  ...
  const L = {
    ja: {
      ...
      'label.hours':'1日の長さ','settings.showSeconds':'秒数表示','settings.language':'言語',
      'footer':'設定は自動で保存されます。','msg.pickTime':'時刻を選択してください','msg.invalidTime':'不正な時刻です',
      'msg.alarmSound':'アラームが鳴りました','noLaps':'ラップなし','noAlarms':'アラームなし'
    },
    en: {
      ...
      'label.hours':'Day length','settings.showSeconds':'Show seconds','settings.language':'Language',
      'footer':'Settings are saved automatically.','msg.pickTime':'Please pick a time','msg.invalidTime':'Invalid time',
      'msg.alarmSound':'Alarm','noLaps':'No laps','noAlarms':'No alarms'
    }
  };
  ...
  secondsLabel.textContent = showSeconds ? (localStorage.getItem('nclock_lang')==='en'?'On':'表示') : (localStorage.getItem('nclock_lang')==='en'?'Off':'非表示');
  ...
  function applyLocalization(){
    ...
    document.querySelectorAll('[data-key]').forEach(el=>{const k=el.getAttribute('data-key'); if(k) el.textContent=t(k);});
    document.querySelectorAll('[data-key-start]').forEach(el=>{el.textContent = running ? t('btn.stop'):t('btn.start');});
  }
    function saveAll(){
  ...
  function renderLaps(){
    lapList.innerHTML='';
    if(laps.length===0){ lapList.innerHTML=`<div style="color:var(--muted);padding:8px">${t('noLaps')}</div>`; return; }
    laps.forEach((t,i)=>{
  ...
  swStart.addEventListener('click',()=>{
    ...
    if(running){
      ...
      swStart.classList.replace('btn-start','btn-stop');
      swLap.disabled=false; swReset.disabled=true;
    }else{
      swStart.textContent=t('btn.start');
      swStart.classList.replace('btn-stop','btn-start');
  ...
  function playAlarmSound(){
    try{
      ...
      for(let i=0;i<6;i++){
        ...
        o.type='sine'; o.frequency.value=880-i*40; o.connect(gain);
  ...
  function tick(now){
    ...
    }else if(mode==='stopwatch'){ display.textContent=formatStopwatch(elapsedMs); }
  ...
  function restore(){
    if(localStorage.getItem('nclock_hours')){ customHours=Number(localStorage.getItem('nclock_hours')); slider.value=customHours; labelHours.textContent=(lang==='en')?`${customHours} h`:`${customHours} 時間`; }
...
if(localStorage.getItem('nclock_show_seconds')) {
  showSeconds = (localStorage.getItem('nclock_show_seconds') === 'true');
  secondsToggle.checked = showSeconds;
  secondsLabel.textContent = showSeconds ? (lang==='en'?'On':'表示') : (lang==='en'?'Off':'非表示');
}
if(localStorage.getItem('nclock_lang')) {
  lang = localStorage.getItem('nclock_lang');
  langSelect.value = lang;
  langSelectSettings.value = lang;
}
setMode(mode);
